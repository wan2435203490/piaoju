...
4. eventTime 用 RFC3339 UTC（如 2026-07-12T11:30:00Z）。票面通常是北京时间（UTC+8），换算成 UTC。
   ...
   train/flight 的 extra.departTime、extra.arriveTime 同样是时间字段，也用 RFC3339 UTC
   （同上换算规则；票面只有时刻时补齐票面日期后再换算），无法确定则填空字符串 ""，绝不写成 "08:32" 这类裸时刻。
...
// extraFieldDesc 部分 extra 字段的 schema 描述：时间字段必须约束成 RFC3339 UTC，
// 否则模型会照票面写裸时刻（"08:32"），前端 datetime 回填解析失败即静默丢值。
var extraFieldDesc = map[string]string{
	"departTime": "出发/起飞时间，RFC3339 UTC（如 2026-07-12T00:32:00Z），未知填空串；不要写裸时刻",
	"arriveTime": "到达时间，RFC3339 UTC（如 2026-07-12T03:00:00Z），未知填空串；不要写裸时刻",
}
...
func outputSchema() map[string]any {
	...
	for _, fields := range extraWhitelist {
		for _, f := range fields {
			...
			if desc, ok := extraFieldDesc[f]; ok {
